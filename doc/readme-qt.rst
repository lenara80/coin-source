MalCoin-qt: Qt4 GUI for MalCoin
===============================
...
Alternatively, install Qt Creator and open the `MalCoin-qt.pro` file.

An executable named `MalCoin-qt` will be built.
...
A warning for people using the *static binary* version of MalCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of MalCoin is linked against libdb5.3.
...
installed causes MalCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
