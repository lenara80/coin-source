...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #MalCoinTEST2\r");
            Send(hSocket, "WHO #MalCoinTEST2\r");
        } else {
            // randomly join #MalCoin00-#MalCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #MalCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #MalCoin%02d\r", channel_number).c_str());
        }
